import { FETCH_EVENTS, CREATE_EVENT, UPDATE_REG_PAGE_MODEL, ADD_SECTION_REG_PAGE } from "../actions/types";
	...
	switch (action.type) {
		...
		case ADD_SECTION_REG_PAGE:
			return state.map((event, index) => {
				if(event.id === action.payload.eventId) {
					return {...event, 
								regPageModel: [...event.regPageModel.slice(0, action.payload.index), 
